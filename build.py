...
class Event:
    ...
    @property
    def anchor(self) -> str:
        """Anchor id compatible with the old hand-written index.html."""
        if self.start:
    ...
    def jsonld(self) -> dict:
        ...
        performers = [
            {"@type": "Person", "name": t["speaker"], **(
                {"url": t["speaker_url"]} if t.get("speaker_url") else {}
            )}
            for t in self.talks
            if t.get("speaker")
        ]
...
def build_ics(events: list[Event]) -> bytes:
    ...
    for ev in events:
        ...
        if ev.talks:
            talk_lines = [
                f"- {t.get('speaker', '')}: {t['title']}".replace("- : ", "- ")
                for t in ev.talks
            ]
...
def build_json_feed(events: list[Event]) -> str:
    items = []
    for ev in events:
        content = f"<p>{ev.display_date}"
        if ev.venue.get("name"):
            content += f" at {ev.venue['name']}"
        content += "</p>"
        if ev.talks:
            content += "<ul>" + "".join(
                f"<li>{t.get('speaker', '')}: {t['title']}</li>" for t in ev.talks
            ) + "</ul>"
        item = {
            "id": ev.url,
            "url": ev.url,
...
def main() -> None:
    ...
    for ev in events:
        ...
        jsonld = json.dumps(ev.jsonld(), indent=2, ensure_ascii=False)
        page_dir.joinpath("index.html").write_text(
            tpl.render(event=ev, jsonld=jsonld, is_upcoming=ev.is_upcoming(today)),
            encoding="utf-8",
        )

    # Feeds
    (out / "events.ics").write_bytes(build_ics(events))
