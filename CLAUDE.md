...
- `templates/*.j2` (Jinja2) → `index.html`/`index.md`, `/events/index.html`/`index.md`,
  `feed.xml`, `sitemap.xml`, `llms.txt`; each `*.html.j2` page template has a `*.md.j2` twin
...
- Static passthrough (the `STATIC` list in `build.py`): `for-speakers/`,
  `for-companies/`, `assets/`, `slides/`, etc.
...
- `templates/llms.txt.j2` links the markdown pages and feeds. When adding or modifying
